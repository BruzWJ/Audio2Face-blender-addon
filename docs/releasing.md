...
only the manifest; another dispatch from that same source verifies and reuses
the exact child instead of creating another commit.
...
reuses an existing draft. Reuse requires the exact date and title. Its target
must either equal the newly verified source or be an ancestor; in the latter
...
- `Microsoft.VisualStudio.Component.VC.Tools.x86.x64` for the canonical
  `vcvarsall.bat` build environment;
- `Microsoft.VisualStudio.Component.VC.14.43.17.13.x86.x64`; and
- `Microsoft.VisualStudio.Component.Windows11SDK.22621`.

The current x64/x86 component supplies Visual Studio's build-environment
infrastructure; the runtime builder still requests and verifies MSVC toolset
`14.43.34808`, so the additional current compiler is never selected as the
release producer. The workflow verifies `vcvarsall.bat`, the locked compiler,
...
an instance containing both `vcvarsall.bat` and the locked compiler. The Ubuntu
...
4. The prepare job tests the source with the dated manifest, confirms the branch
   ...
   the stamped source SHA. Another dispatch from the same source verifies and
   reuses the exact stamp if it already exists. The job then creates or verifies
   the dated draft and retains its numeric ID. GitHub generates the notes from
...
the failed run's original workflow and source. A new run on the same UTC date
reuses the matching dated draft, requires its old source to be an ancestor,
regenerates its notes, retargets it to the newly verified source SHA, and
replaces any same-named assets. Drafts for a different date or a dated tag at a
different commit remain intentional blockers and must be resolved before
